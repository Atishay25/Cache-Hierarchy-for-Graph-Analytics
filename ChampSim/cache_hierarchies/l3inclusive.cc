...
//dram - 0, llc - 1
int CACHE::writeback_to_memory(uint32_t fill_cpu,uint64_t address,uint32_t instr_id, int isllc){
  int do_fill = 1;
  ...
  bool temp;
  if (isllc)
    temp = uncore.LLC.get_occupancy(2, address) == uncore.LLC.get_occupancy(2, address);
  else
    temp = uncore.DRAM.get_occupancy(2, address) == uncore.DRAM.get_size(2, address);
  if (temp) {

    // lower level WQ is full, cannot replace this victim
    do_fill = 0;
    if(isllc)
      uncore.LLC.increment_WQ_FULL(address);
    else
    ...
    cout << " victim_addr: " << block[set][way].tag << dec << endl; });
      ...
      if(isllc){
        writeback_packet.fill_level = FILL_LLC;
        uncore.LLC.add_wq(&writeback_packet);
      }
      else{
        ...
        if (cache_type==IS_LLC){
          if (block[set][way].valid) {
            ...
            for ( int i = 0 ; i < NUM_CPUS ; i++ ){
              ...
              int l1status;
              if(l1dstatus == 3){
                l1container = &ooo_cpu[i].L1I;
                l1status = l1istatus;
              }else{
                l1container = &ooo_cpu[i].L1D;
                l1status = l1dstatus;
              }
              if(l1status == 2){
...
// 1 for clean, 2 for dirty, 3 for invalid
int CACHE::entry_type(uint64_t inval_addr){
